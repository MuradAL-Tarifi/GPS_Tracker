...
$(document).ready(function () {
    $('#logoInput').change(function (e) {
        var isValidFileImage = fileSizeAndExtenionPngValidation(this, 50);
        if (isValidFileImage) {
    ...
    $("#AddUpdateForm").on("submit", function (event) {
        ...
        if ($("#AddUpdateForm").valid()) {
            var ajaxConfig = {
                url: hostName + "/SystemSetting/AddUpdate",
...
function fileSizeAndExtenionPngValidation(oInput, maxSizejpeg) {
    var validPng = "png";
    var isValid = false;
    if (oInput.type == "file") {
        var fileName = oInput.value;
        if (fileName.length > 0)
            if (fileName.substr(fileName.length - validPng.length, validPng.length)
                .toLowerCase() == validPng.toLowerCase()) {
                isValid = true;
            }
            if (!isValid) {
                alert("Sorry, this file is invalid, allowed extension is: " + validPng);
                oInput.value = "";
                isValid = false;
            }
